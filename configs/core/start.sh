...
if [ "$role" = "ingest" ]; then
    ...
    cd /var/www/html && php artisan cache:clear && php artisan config:cache
    supervisord -n --configuration /etc/supervisor/supervisord.conf
elif [ "$role" = "beam" ]; then
    echo "Running beam..."
    (cd /var/www/html && php artisan cache:clear && php artisan config:cache && php artisan platform:process-beam-claims)
else
